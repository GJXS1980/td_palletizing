...
class TA_Mission_Test:
    def __init__(self):
        ...
        # 抓取识别点,单位mm	
        self.grasp_x = int(rospy.get_param("~grasp_x", "35.0"))
        self.grasp_y = int(rospy.get_param("~grasp_y", "0.0"))
        self.grasp_z = int(rospy.get_param("~grasp_z", "0.0"))
        
        # 侧边放置位置点,单位mm
        self.push_x = int(rospy.get_param("~push_x", "60.0"))
        self.push_y = int(rospy.get_param("~push_y", "410.0"))
        self.push_z = int(rospy.get_param("~push_z", "210.0"))

        # 货车默认放置点,单位mm
        self.real_push_x = int(rospy.get_param("~real_push_x", "15.0"))
        self.real_push_y = int(rospy.get_param("~real_push_y", "-45.0"))
        self.real_push_z = int(rospy.get_param("~real_push_z", "380.0"))
    ...
    def get_mission(self):
        while True:
            if self.TA_Mqtt.TA_Mission_Msg == 1:
                ...
                print("开始上货")
                # 移动到抓取识别点
                if self.set_ta_pose([self.grasp_x, self.grasp_y, self.grasp_z]):
                    try:
                        ...
                        if self.set_ta_pose([0.0, 0.0, 100.0]):
                            # 放置点
                            if self.set_ta_pose([self.push_x, self.push_y, self.push_z]):#395
            ...
            if self.TA_Mqtt.TA_Mission_Msg == 2:
                ...
                if self.vision_mode == 1:
                    ...
                    # 移动到放置识别点
                    if self.set_ta_pose([self.grasp_x, self.grasp_y, self.grasp_z]):#210
                        # 识别放置坐标并记录(普通ar码识别)
                        try:
                ...
                # 放置点抓取
                if self.set_ta_pose([self.push_x, self.push_y, self.push_z]):
                    self.set_ta_grasp_control(1)
                    time.sleep(1.0)
                    while True:
                    ...
                    if self.set_ta_pose([0.0, 0.0, 10.0]):  # 抓到后先往上
                        ...
                        if self.set_ta_pose([self.grasp_x, self.grasp_y, self.grasp_z]):
                            if self.vision_mode == 1 and self.push_target_pose[2] != 0.0:
                                ...
                                if self.set_ta_pose([self.real_push_x, self.real_push_y, self.real_push_z]): # 盲放位置点 60 70 210
                                    print("push target")
